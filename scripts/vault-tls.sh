...
TLS_PATH="../docker/vault/config/tls"
...
# Sign the CSR
openssl x509 -extfile request.cfg -extensions ext -req -in $TLS_PATH/vault.csr -CA $TLS_PATH/vault_ca.crt -CAkey $TLS_PATH/vault_ca.key -CAcreateserial -out $TLS_PATH/vault.crt -days 3650
